...
N, D = X.shape  # bestimmt die Dimensionen von X : N sind die Anzahl Zeilen und D die Anzahl Spalten => N=60000 , D= 784
Xtest, ytest = mndata.load_testing()
ytest = np.array(ytest, dtype="uint8")
Xtest = np.array([np.array(x) for x in Xtest], dtype="uint8")
Ntest = Xtest.shape[0] # Anzahl an Zeilen im Testdatensatz, hier 10000 Zeilen

# %%
# in dem folgenden Abschnitt wird einen kleinerer Trainingsdatensatz mit 100 Beispielen 
# und ein Testdatensatz mit 10 Beispielen erstellt
p = np.zeros(0, dtype="int")
for c in range(10): # für jede Ziffer 0 bis 9 werden zufällig 100 Datenpunkte aus dem Trainingsdatensatz ausgewählt 
    p = np.append(p, np.random.choice(np.where(y == c)[0], size=100, replace=False))
X_s = X[p, :] # mit den eben zufällig ausgewählten 100 Beispielen wird ein neuer Trainingsdatensatz erstellt
y_s = y[p]
N_s = X_s.shape[0]
p = np.zeros(0, dtype="int")
for c in range(10):
...
# %%
# der folgende Abschnitt ist nur zur Überprüfung ob der Code in Jypiter Notebook oder sonst wo ausgeführt wird
# und je nachdem wird dann die entsprechende Bibliothek zur Darstellung von Diagrammen ausgesucht
...
# %%
# Vorbereitung zum plotten von Diagrammen indem eventuell noch vorhandende Bilder gelöscht werden 
# oder schon eine Vorlage erstellt wird
def nextplot():
    if inTerminal:
