...
def nb_train(X, y, alpha=1, K=None, C=None):
    ...
    Parameters
    ----------
    X : ndarray of shape (N,D) => hier 60000 x 784 Matrix mit allen Trainingsdaten 
        Design matrix.
    y : ndarray of shape (N,) =>  hier 60000 x 1 Vektor mit allen Labels der Trainingsdaten
        Class labels.
    alpha : int
        Parameter for symmetric Dirichlet prior (Laplace smoothing) for all
        fitted distributions.
    K : int => sind die Anzahl Farbtöne, die in den Bildern vorhanden sind 
       # hier ist K = 256 , denn die Graubilder haben Pixelwerte von 0 bis 255
        Each feature takes values in [0,K-1]. None means auto-detect.
    C : int => sind die Anzahl Ziffern, die in den Bildern vorkommen => hier C=10 , denn Ziffern von 0 bis 9
